...
def main():
    ...
    for train_index, test_index in skf.split(docs, labels):
        ...
        for title, pred_y, true_y in misclassifications:
            if pred_y == 'opposes':
                if true_y == 'supports':
                    pred_oppose_true_support.append(title)
                if true_y == 'other':
                    pred_oppose_true_other.append(title)

            elif pred_y == 'supports':
                if true_y == 'other':
                    pred_support_true_other.append(title)
                if true_y == 'opposes':
                    pred_support_true_oppose.append(title)

            elif pred_y == 'other':
                if true_y == 'supports':
                    pred_other_true_support.append(title)
                if true_y == 'opposes':
                    pred_other_true_oppose.append(title)
    ...
    model = RelationshipClassifier(directional=True, epochs=15)
