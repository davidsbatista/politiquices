...
def main():
    ...
    with open(sys.argv[1], 'rt') as f_in:
        ...
        for line in f_in:
            ...
            _, persons = rule_ner.tag(title, all_entities=False)
            if any(person in persons for person in ner_ignore):
                ner_ignored.write({"title": title, "entities": persons})
                continue

            if len(persons) == 2:

                # NER
                title_PER = title.replace(persons[0], "PER").replace(persons[1], "PER")
                ...
                more_entities.write({"title": title, "entities": persons})

            else:
                no_entities.write({"title": title, "entities": persons})
